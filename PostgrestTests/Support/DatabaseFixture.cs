...
namespace PostgrestTests.Support;
...
{
    // The dummy_data migration (supabase/migrations/1749515175_dummy_data.sql). Its statements are stored
    // verbatim in supabase_migrations.schema_migrations, so we replay them instead of duplicating the seed.
    // The version is inlined below (not a parameter): Npgsql does not substitute placeholders inside a
    ...
    private const string ResetScript = """
        ...
        BEGIN
            FOR stmt IN
                SELECT unnest(statements) FROM supabase_migrations.schema_migrations WHERE version = '1749515175'
            LOOP
                EXECUTE stmt;
            END LOOP;
        END $$;
