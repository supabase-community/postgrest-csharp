...
namespace PostgrestTests
{
	...
	public class ClientTests
	{
		...
		public async Task TestMatchFilter()
		{
			...
			var client = new Client(BaseUrl);
			var usersResponse = await client.Table<User>().Get();
			var expected = usersResponse.Models.Where(u => u.Username == "kiwicopple" && u.Status == "OFFLINE").ToList();
		...
		public async Task TestNullValueHandlingOnColumn()
		{
			...
			Assert.AreEqual(now.ToString(CultureInfo.CurrentCulture), insertResponse.Models[0].DateTimeValue.ToString());
			Assert.AreEqual(now.ToString(CultureInfo.CurrentCulture), insertResponse.Models[0].DateTimeValue1.ToString());
		...
		public async Task TestEnums()
		{
			...
			await client.Table<Todo>().Filter(x => x.Status, Operator.Equals, GetEnumMemberAttrValue(Todo.TodoStatus.IN_PROGRESS)).Get();
		}
